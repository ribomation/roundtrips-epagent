...
{ ### $roundTrips->extract()
    ...
    is($result{time}, '4860', 'test data: time');
...
{ ### $roundTrips->processLine($logline, $metrics)
    ...
    is($metrics->{'/rest|/secure/mediarentals:Average Time [ms]'}->value, 0, 'process line: avg time');
...

{
    ...
    $roundTrips->processLine($logline, $metrics);
    is($metrics->{'/rest|/livechannels:Average Time [ms]'}->value, 65, 'process line: avg time');
    ...
    $roundTrips->processLine($logline, $metrics);
    is($metrics->{'/npvr|/rest/v1/recordings/_/programs:Average Time [ms]'}->value, 12, 'process line: avg time');
}
